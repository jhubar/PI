...
class SEIR():
    ...
    def differential_bis(self, state, time, beta, gamma, sigma, hp, hcr):
        ...
        dI = (1 - hp) * (E * sigma) - (
                    gamma * I)  # Note: on retire la proportion des hospitalisés, ils ne participent plus à la contagion
        dH = hp * (E * sigma) - hcr * H
        dR = gamma * I + hcr * H
    ...
    def fit(self, dataset):
        ...
        res = minimize(self.SSE, np.asarray(start_values), args=(initial_state, time, dataset, method),
    ...
    def fit_scipy(self, dataset):
        ...
        # Set initial state:
        initial_state = (
        1000000 - dataset[0][1] - 3 * dataset[2][1], 3 * dataset[2][1], dataset[0][1], dataset[0][4], 0)
        start_values = [self.beta, self.gamma, self.sigma, self.hp, self.hcr]
