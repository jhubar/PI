...
class SEIR():
    ...
    def dataframe_smoothing(self, df):
        ...
        # ATTENTION: moyenne realisee le 8/11: critical = H / 7.15
        for i in range(0, smoothed.shape[0]):
            print('{}, {}'.format(smoothed[i][5], smoothed[i][3]))
            smoothed[i][5] = float(smoothed[i][3] / 7.15)
            print(smoothed[i][5])
    ...
    def import_dataset(self, target='covid_20'):
        ...
        if target == 'covid_20':
            ...
            y_raw = self.raw_dataset['num_positive'].to_numpy(dtype=np.float64)
            y_smooth = self.dataframe['num_positive'].to_numpy(dtype=np.float64)
