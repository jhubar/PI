...
function loadData(){
  ...
  $.get(url,function(data){
    ...
    for(var i=0;i<result.length-1;i++){
      ...
      dataL.push(result[i].num_fatalities);
      dataOverfit.push(result[i].num_fatalities);
      dataUnderfit.push(result[i].num_fatalities);
      dataLinearfit.push(result[i].num_fatalities);
    }
    ...
    for(var i=result.length-1;i<label.length;i++){
      tmpLinearfit = (0.01) * parseInt(label[i-1]);
      if(i%2 == 0){
        tmpOverfit = 0.04 + parseInt(tmpLinearfit);
        tmpUnderfit = parseInt(tmpLinearfit) - 0.004;
      }
      else{
        tmpOverfit = 0.03 + parseInt(tmpLinearfit);
        tmpUnderfit = parseInt(tmpLinearfit)-0.003;
      }
